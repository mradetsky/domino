...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "@uirouter/core"], factory);
    }
})(function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var core_1 = require("@uirouter/core");
    var notInArray = function (arr) { return function (item) { return !core_1.inArray(arr, item); }; };
    ...
    var isChildOfAny = function (_parents) {
        return function (node) {
            return _parents.map(function (parent) { return isChildOf(parent)(node); }).reduce(core_1.anyTrueR, false);
        };
    };
    ...
    var StickyStatesPlugin = (function (_super) {
        ...
        function StickyStatesPlugin(router) {
            ...
            _this._getInactive = function (node) {
                return node && core_1.find(_this._inactives, function (n) { return n.state === node.state; });
            };
        ...
        StickyStatesPlugin.prototype._defineStickyPaths = function () {
            ...
            this.pluginAPI._definePathType("inactivating", core_1.TransitionHookScope.STATE);
            this.pluginAPI._definePathType("reactivating", core_1.TransitionHookScope.STATE);
        };
        StickyStatesPlugin.prototype._defineStickyEvents = function () {
            var paths = this.pluginAPI._getPathTypes();
            this.pluginAPI._defineEvent("onInactivate", core_1.TransitionHookPhase.RUN, 5, paths.inactivating, true);
            this.pluginAPI._defineEvent("onReactivate", core_1.TransitionHookPhase.RUN, 35, paths.reactivating);
        };
        ...
        StickyStatesPlugin.prototype._calculateExitSticky = function (tc, trans) {
            ...
            if (!core_1.isArray(exitSticky))
                exitSticky = [exitSticky];
            var $state = trans.router.stateService;
            var states = exitSticky
                .map(core_1.assertMap(function (stateOrName) { return $state.get(stateOrName); }, function (state) { return "State not found: " + state; }))
                .map(function (state) { return state.$$state(); });
            var potentialExitingStickies = this._inactives.concat(tc.inactivating).reduce(core_1.uniqR, []);
            ...
            var exitingInactives = states.map(core_1.assertMap(findInactive, notInactiveMsg));
            ...
            exiting.map(core_1.assertMap(function (node) { return !core_1.inArray(tc.to, node); }, inToPathMsg));
            return exiting;
        };
        StickyStatesPlugin.prototype._calculateStickyTreeChanges = function (trans) {
            ...
            var inactiveFromPath = tc.retained.concat(tc.entering.map(function (node) { return _this._getInactive(node) || null; })).filter(core_1.identity);
            var simulatedTC = core_1.PathUtils.treeChanges(inactiveFromPath, tc.to, trans.options().reloadState);
            ...
            var childrenOfToState = inactives.filter(isChildOf(core_1.tail(tc.to)));
            ...
            trans.onSuccess({}, function () {
                tc.exiting.forEach(core_1.removeFrom(_this._inactives));
                tc.entering.forEach(core_1.removeFrom(_this._inactives));
                tc.reactivating.forEach(core_1.removeFrom(_this._inactives));
                tc.inactivating.forEach(core_1.pushTo(_this._inactives));
            });
            ...
            exitSticky.filter(notInArray(tc.exiting)).forEach(core_1.pushTo(tc.exiting));
            // Also process the active sticky states that are about to be inactivated, but should be exited
            exitSticky.filter(core_1.inArray(tc.inactivating)).forEach(core_1.removeFrom(tc.inactivating));
            return tc;
        };
        StickyStatesPlugin.prototype._addDefaultTransitionOption = function () {
            core_1.defaultTransOpts.exitSticky = [];
        };
